...
GCGraph::GCGraph( unsigned int vtxCount, unsigned int edgeCount )
{
...
void GCGraph::create( unsigned int vtxCount, unsigned int edgeCount )
{
...
void GCGraph::addEdges( int i, int j, double w, double revw )
{
...
void GCGraph::addTerminalWeights( int i, double sourceW, double sinkW )
{
    CV_Assert( i>=0 && i<(int)vtcs.size() );
...
bool GCGraph::inSourceSegment( int i )
{
    CV_Assert( i>=0 && i<(int)vtcs.size() );
    return vtcs[i].t == 0;
}
